...
Table::Table(std::fstream &fin) {
    ...
    while(std::getline(fin, row)) {
        grid_.push_back(std::vector<bool>());
...
inline bool Table::neighbourCellValue(size_t row, size_t column, 
                               int direction_row, int direction_column) {
    size_t neighbour_row = (row + height() + direction_row) % height();
    size_t neighbour_column = (column + width() + direction_column) % width();
    return grid_[neighbour_row][neighbour_column];
}
